...
function renderTrain() {
  ...
  return `${daySwitcher()}<div class="session-heading"><div><h2>${state.activeDay.replace("DÍA ", "Día ")}</h2><p>Registra cada ejercicio antes de finalizar.</p></div><span class="target">${plan.length} ejercicios</span></div><section class="exercise-list">${plan.map((item, planIndex) => {
    ...
    return `<article class="exercise-item" data-workout-id="${exercise.id}"><div class="exercise-title"><div><h3>${escapeHtml(exercise.name)}</h3><p>${escapeHtml(exercise.muscle)}</p></div><span class="target">${item.sets} x ${item.reps}<br>${escapeHtml(item.resistance)}</span></div><p>${escapeHtml(exercise.summary)}</p>${exerciseDetails(exercise)}<div class="record-grid"><div><span>Repeticiones objetivo</span><button type="button" class="plan-exercise-picker" data-goto-plan="reps" data-workout-id="${exercise.id}" data-plan-index="${planIndex}" aria-label="Editar repeticiones en Plan">${escapeHtml(item.reps)}</button></div><div><span>Resistencia / peso</span><button type="button" class="plan-exercise-picker" data-goto-plan="resistance" data-workout-id="${exercise.id}" data-plan-index="${planIndex}" aria-label="Editar resistencia en Plan">${escapeHtml(item.resistance)}</button></div></div><div class="exercise-actions"><output class="sets-counter" aria-label="Series realizadas">${completed}</output><button class="guide-play" data-action="start-guide" data-exercise-id="${exercise.id}" data-sets="${escapeHtml(item.sets)}" data-repetitions="${escapeHtml(item.reps)}" aria-label="Iniciar guía para ${escapeHtml(exercise.name)}"><span aria-hidden="true">▶</span> Guía</button><button class="outline-button" data-action="show-videos" data-exercise-id="${exercise.id}">Vídeos</button>${RATING_OPTIONS.map((rating) => `<button class="rating ${entry.rating === rating ? "selected" : ""}" data-rating="${rating}">${rating}</button>`).join("")}</div></article>`;
