...
- Historial: se conservan todas las sesiones finalizadas, incluidas varias del mismo `day` (antes `finish` reemplazaba la del día); tope `MAX_HISTORY = 30` (`js/constants.js:7`) [^constants] aplicado de forma centralizada en `save()` (`js/state.js:23-30`), que recorta a las más recientes por fecha con el comparador de `sortedHistory()`. Backup/restore también quedan acotados al pasar por `save()`. `save()` además sanea el historial con `normaliseHistory()` (tipos coercidos, `day` solo `DÍA 1..7`, `guide` numérico) sin rechazar backups v2/v3; `reuse` valida el día contra `configuredDays()`.
