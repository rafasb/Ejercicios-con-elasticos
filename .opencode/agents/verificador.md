...
Al gestionar `python3 -m http.server 4173`, PROHIBIDO `pkill -f "http.server 4173"` (el patrón coincide con su propia shell y cuelga la llamada).
Usa `pkill -f "http[.]server 4173"` o guarda el PID (`echo $!`) y `kill <pid>`.
Lanza el parado en llamada separada del arranque.
